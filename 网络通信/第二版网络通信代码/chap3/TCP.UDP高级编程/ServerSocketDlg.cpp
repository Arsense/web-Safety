...
///////////////////////////////////////////////////////////////////////////////
// PickNextAvailable : this is useful only for TCP socket该函数只对TCP使用
...
bool CServerSocketDlg::StartServer()
{
	...
	if (m_pCurServer != NULL)
	{
		...
		if (bSuccess && m_pCurServer->WatchComm())
		{
			...
			CString strMsg = _T("服务器: ") + strServer;
					strMsg += _T(", @IP地址: ") + strAddr;
					strMsg += _T(" 运行在端口 ") + m_strPort + CString("\r\n");
...
BOOL CServerSocketDlg::OnInitDialog()
{
	...
	SetIcon(m_hIcon, TRUE);			// 设定大图标
	SetIcon(m_hIcon, FALSE);		// 设定小图标
	...
	for(int i=0; i<MAX_CONNECTION; i++)
	{
		...
		m_SocketManager[i].SetServerState( true );	// 作为服务器运行
	}
