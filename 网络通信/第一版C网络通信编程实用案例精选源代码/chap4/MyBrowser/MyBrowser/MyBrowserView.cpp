...
BEGIN_MESSAGE_MAP(CMyBrowserView, CHtmlView)
	...
	ON_COMMAND(ID_GET_HTML_FILE, OnGetHtmlFile)
	//}}AFX_MSG_MAP
...
void CMyBrowserView::OnGetHtmlFile() 
{
	// TODO: Add your command handler code here
	GetHtmlDocument();		//
	...
	if(silent)
	{
		AfxMessageBox("cannot show dialog box");
	}
	else
	{
		AfxMessageBox("can show dialog box");
	}
...
//	SetFullScreen(TRUE);// no use
//	SetTheaterMode(TRUE);// no use

	GetBusy();	// no use

	SetHeight(1000);	// no use
	str=GetType();		//html ÎÄµµ
	AfxMessageBox(str);
	SetVisible(FALSE);
