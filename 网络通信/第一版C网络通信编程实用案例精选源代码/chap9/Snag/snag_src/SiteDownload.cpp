...
BEGIN_MESSAGE_MAP(CSiteDownloadApp, CWinApp)
	//{{AFX_MSG_MAP(CSiteDownloadApp)
...
// or later)
BOOL CSiteDownloadApp::HasNewShell()
{
...
BOOL CSiteDownloadApp::AlreadyRunning()
{
	BOOL bFound = FALSE;
...
/////////////////////////////////////////////////////////////////////////////
// CSiteDownloadApp construction

CSiteDownloadApp::CSiteDownloadApp()
{
}

/////////////////////////////////////////////////////////////////////////////
// The one and only CSiteDownloadApp object

CSiteDownloadApp theApp;

/////////////////////////////////////////////////////////////////////////////
// CSiteDownloadApp initialization

// Initialize the app -- check to see if is already running, make sure we're
// running the right OS
BOOL CSiteDownloadApp::InitInstance()
{
	...
	CRect rect;
	this->GetMainWnd()->GetWindowRect(&rect);
	m_pMainWnd->SetWindowPos(NULL,-1,-1,rect.right-rect.left,rect.bottom-rect.top,SWP_NOMOVE);
...
void CSiteDownloadApp::UpdateMRU(CCmdUI *pCmdUI)
{
...
//
BOOL CSiteDownloadApp::ProcessCommandLine()
{
	CString strURL;
	CConfigure Options;
	...
	if(__argc > 1)
	{
		...
		CString strFileName;
		CString strPath = CInternetDownload::SplitFileName(__argv[1],CInternetDownload::PATH);
		if(strPath.IsEmpty())
			strFileName = CString("Projects\\");
		strFileName += __argv[1];

		// Try to open the document that the user specified on the command line
		CMyDoc *pDoc = (CMyDoc *) m_pDocManager->OpenDocumentFile(strFileName);
...
void CSiteDownloadApp::OnAppAbout()
{
	CAboutDlg aboutDlg;
	aboutDlg.DoModal();
}
