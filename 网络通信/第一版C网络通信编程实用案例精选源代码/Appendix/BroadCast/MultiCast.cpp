...
{
	if(m_bConnected){
		AfxMessageBox("Has Successfully Create the socket!\n");
		return FALSE;
	}
	m_hWnd = hWnd;
	int ret;
	...
	if(ret == SOCKET_ERROR){
		AfxMessageBox("setsockopt:%d",WSAGetLastError());
		return FALSE;
    } 
    // 将套接字绑扎到用户指定端口及默认的接口
    ...
    if (ret == SOCKET_ERROR){
		AfxMessageBox("SAIoctl(SIO_MULTICAST_SCOPE):%d\n",WSAGetLastError());
		return FALSE;
	}
	...
	if(ret == SOCKET_ERROR){
		m_bConnected = FALSE;
		AfxMessageBox("WSAStringToAddress:%d\n",WSAGetLastError());
		return FALSE;
	}
	...
	if(ret == SOCKET_ERROR){
		AfxMessageBox("WSAAsyncSelect():%d\n", WSAGetLastError());
		m_bConnected = FALSE;
		return FALSE;
	}
	m_bConnected = TRUE;
