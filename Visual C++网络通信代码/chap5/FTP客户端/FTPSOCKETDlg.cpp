...
CFTPSOCKETDlg::CFTPSOCKETDlg(CWnd* pParent /*=NULL*/)
	: CDialog(CFTPSOCKETDlg::IDD, pParent)
{
	//{{AFX_DATA_INIT(CFTPSOCKETDlg)
	m_PWD = _T("");
	m_Server = _T("");
	m_UID = _T("");
	m_Message = _T("");
...
void CFTPSOCKETDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CFTPSOCKETDlg)
...
BEGIN_MESSAGE_MAP(CFTPSOCKETDlg, CDialog)
	//{{AFX_MSG_MAP(CFTPSOCKETDlg)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
...
//}}AFX_MSG_MAP
	...
	ON_MESSAGE(WM_SETFILE,CFTPSOCKETDlg::OnInsertLocFile)
...
/////////////////////////////////////////////////////////////////////////////
// CFTPSOCKETDlg message handlers
...
void CFTPSOCKETDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
...
HCURSOR CFTPSOCKETDlg::OnQueryDragIcon()
{
	return (HCURSOR) m_hIcon;
}

void CFTPSOCKETDlg::OnButtonConnect() 
{
	int ndx=0;	
	CString str;
	//将控件中的数据更新到对应的变量中
...
void CFTPSOCKETDlg::SetFileColumns(CListCtrl *pListCtrl)
{
   	static struct
	{
		LPSTR pszText;
...
void CFTPSOCKETDlg::SetServerFileColumns(CListCtrl *pListCtrl)
{
   	static struct
	{
		LPSTR pszText;
...
BOOL CFTPSOCKETDlg::SetImageList(CListCtrl* pListCtrl,CComboBoxEx* pCombo)
{
...
int CFTPSOCKETDlg::InsertLocDir(CString &str, int iIcon, int iIconSel)
{
...
void CFTPSOCKETDlg::GetFileIcon(CString& fileName,int* iIcon,int* iIconSel)
{
...
void CFTPSOCKETDlg::GetLocCurrentDirectory()
{
    CString str;
...
BOOL CFTPSOCKETDlg::BeginFindLocFile()
{
...
void  CFTPSOCKETDlg::FindLocFile()
{
...
LRESULT CFTPSOCKETDlg::OnSetLocRedrawFlag(WPARAM wParam,LPARAM lParam)
{
	if(m_bHaveDotFlag)
	{
		CString str="..";
...
LRESULT CFTPSOCKETDlg::OnSetLocDirectoryDisplay(WPARAM wParam,LPARAM lParam)
{
	CString str;
	str=szLocDirectory;
...
LRESULT CFTPSOCKETDlg::OnReDisplayLocFile(WPARAM wParam,LPARAM lParam)
{
...
LRESULT CFTPSOCKETDlg::OnInsertLocFile(WPARAM wParam,LPARAM lParam)
{
...
LRESULT CFTPSOCKETDlg::OnSetLocDir(WPARAM wParam,LPARAM lParam)
{
...
void CFTPSOCKETDlg::OnInsertFile(CListCtrl* pListCtrl,LPVOID pIn,LPARAM lParam)
{
...
void CFTPSOCKETDlg::OnSelchangeComboboxexLocal() 
{
	CString str;
    char lpBuffer[MAX_PATH];
...
void CFTPSOCKETDlg::SetServerDir (CString& str)
{
...
void CFTPSOCKETDlg::AddLine(CString strLine)
{
...
BOOL CFTPSOCKETDlg::AddItem(CListItem *pItem)
{
...
void CFTPSOCKETDlg::OnDblclkListServerfile(NMHDR* pNMHDR, LRESULT* pResult) 
{
...
void CFTPSOCKETDlg::CD(CString dir)
{
	int ndx=0;
	dir.TrimLeft();
	dir.TrimRight();
...
void CFTPSOCKETDlg::UpLoadFile(CString LocalFilePath,CString LocalFileName,bool pasv,bool get)
{
	CString str;
	str=szFtpDirectory;
	if (str.Right(1)!="/")
...
void CFTPSOCKETDlg::DownLoadFile(CString ServerFilePath,CString ServerFileName,bool pasv,bool get)
{
	CString str;
	str=szLocDirectory;
	if (str.Right(1)!="\\")
