...
CString CTrayIcon::GetTooltipText()
{
	CString strText;
	if (m_bEnabled)
		strText = m_tnd.szTip;
...
LRESULT CTrayIcon::OnTrayNotification(UINT wParam, LONG lParam) 
{
	...
	else if (WM_LBUTTONDBLCLK == LOWORD(lParam))
	{
		if (!menu.LoadMenu(m_tnd.uID)) return 0;
		if (!(pSubMenu = menu.GetSubMenu(0))) return 0;
...
BOOL CTrayIcon::SetMaxWidth(int iWidth)
{
	if (m_bEnabled)
	::SendMessage(m_tnd.hWnd, TTM_SETMAXTIPWIDTH, 0, iWidth);
	return m_bEnabled;
}
