...
bool TransmitFile(SOCKET& var_socket, char* Name, int Index);
				...
				send(s,Buffer,strlen(Buffer),0);
					...
					if (strstr(Buffer,"RETR "))
								...
								/*
									...
									if (!TransmitFile(s, Name, Index))
									{
											printf("Transmit file error\n");
									}
								}
...
bool TransmitFile(SOCKET& var_socket, char* Name, int Index)
{
	char* FileBuffer = (char*)malloc(sizeof(char)*100);
	...
	pPerson pTemp = pFileHead->pPersonHead;
	pFileInfor pTemp1 = pFileHead;
	pUserInfor pTemp2 = pUserHead;
	
	while (pTemp2 != NULL)
	{
		if (strcmp(Name, pTemp2->NameBuffer) == 0)
		{
			strcpy(FileDirectory, pTemp2->UserDirec);
	...
	//strstr(FileDirectory, "*.*");
//	strncpy(FileAddr, FileDirectory);
	
	while ((FileDirectory[i]) != '*')
	{
		FileAddr[j++] = FileDirectory[i];
...
//	strcat(FileAddr, "\\");
	...
	while (pTemp1 != NULL)
	{
		if ((strcmp(Name, pTemp->NameBuffer)) == 0)
		{
			while (pTemp!= NULL)
			{
				if ((pTemp->Index + 1) == Index)
				{
					FileLength = pTemp->FileLength - '0';
					
					strcpy(FileName, pTemp->FileName);
					
					break;
				}
				
				pTemp = pTemp->pPersonNext;
			}
			
			break;
		}
		
		pTemp1 = pTemp1->pFileInforNext;
	}
...
//	strcat(FileAddr, FileName);
	...
	BOOL bRet = ReadFile(hFile, FileBuffer, 100, &dwRead, NULL);

	strcpy(SendBuffer, FileBuffer);
	
	if (bRet == FALSE)
	{
		printf("Packed file error\n");
	}
	
	int Ret = send(var_socket, SendBuffer,strlen(SendBuffer), 0);
